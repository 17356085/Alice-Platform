"""
Browser runtime implementations — BrowserRuntime and RemoteBrowserRuntime.

Extracted from platform.runtime to eliminate circular dependencies.
Skills never know if it's BrowserUse, Playwright, or API underneath.

Usage:
    from aitest.runtime.browser import BrowserRuntime
    rt = BrowserRuntime(base_url="https://example.com")
...
from aitest.runtime.base import Runtime
...
class BrowserRuntime(Runtime):
    ...
    def _build_capabilities(self):
        """Register browser capability adapters supplied by the composition root."""
        return super()._build_capabilities()
        ...
        try:
            ...
            if "RESULT_CODE_HUNG" in err_msg or "timeout" in err_msg.lower():
                if wait_until != "domcontentloaded":
                    logger.warning(
                        "Navigation hung with wait_until=%s; falling back to "
                        "domcontentloaded. URL=%s",
                        wait_until,
                        url,
                    )
