# [LAYER:Adapter/Event] 从 aitest/audit_engine/event_bus.py 搬入
"""
...
class KnowledgeAgentSubscriber:
    ...
    用法:
        sub = KnowledgeAgentSubscriber()  # provider auto-resolved
    ...
    def __init__(self, provider: str = None, auto_process: bool = True):
        if provider is None:
...
class ReviewAgentSubscriber:
    ...
    用法:
        sub = ReviewAgentSubscriber(auto_trigger=False)  # provider auto-resolved
    ...
    def __init__(self, provider: str = None, auto_trigger: bool = False):
        if provider is None:
            from aitest.runtime.config import config
            provider = config.resolve_llm_provider()
        self.provider = provider
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        logger.info("Usage: python event_bus.py emit|listen|process|watch|clean")
        logger.info(f"Events dir: {EVENT_DIR}")
        logger.info(f"Subscribers active: {sum(len(v) for v in _subscribers.values())}")
        sys.exit(0)
    ...
    if cmd == "emit":
        if len(sys.argv) < 3:
            logger.info("Usage: python event_bus.py emit <EventType> [key=value ...]")
            logger.info(f"Types: {list(EVENT_ACTIONS.keys())}")
            sys.exit(1)
        ...
        logger.info(f"Emitted: {evt.id} ({evt.type})")

    elif cmd == "listen":
        pending = list_pending()
        logger.info(f"Pending events: {len(pending)}")
        for evt in pending:
            logger.info(f"  [{evt.type}] {evt.id} — {evt.data}")

    elif cmd == "process":
        results = process_pending()
        logger.info(f"Processed: {len(results)} events")
        for r in results:
            logger.info(f"  [{r['type']}] → {r['action']}")
            logger.info(f"    Prompt: {r['prompt'][:120]}")

    elif cmd == "watch":
        # 持续监听模式 — Knowledge Agent 作为事件订阅者运行
        interval = int(sys.argv[2]) if len(sys.argv) > 2 else 60
        logger.info(f"Knowledge Agent watching for events (interval={interval}s)...")
        logger.info(f"Events dir: {EVENT_DIR}")
        logger.info("Press Ctrl+C to stop.")

        sub = KnowledgeAgentSubscriber(auto_process=True)
        sub.activate()
        logger.info(f"Subscriber activated: {sub._active}")
        logger.info(f"Auto-process: {sub.auto_process}")
        ...
        try:
            while True:
                ...
                if pending:
                    ...
                    for r in results:
                        logger.info(f"  ✅ [{r['type']}] → {r['action']}")
                        logger.info(f"     {r['prompt'][:100]}")
                    logger.info(f"  Total processed: {sub.processed_count}")
                else:
                    logger.info(f"[{time.strftime('%H:%M:%S')}] No pending events.", end="\r")

                time.sleep(interval)
        except KeyboardInterrupt:
            logger.info("\n\nKnowledge Agent stopped.")
            sub.deactivate()
            logger.info(f"Total events processed: {sub.processed_count}")

    elif cmd == "clean":
        cleaned = cleanup_old_events()
        logger.info(f"Cleaned {cleaned} old processed events")
