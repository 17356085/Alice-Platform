...
def cmd_check(args):
    ...
    if args.consistency:
        ...
        from aitest.consistency_checker import run_all_checks
...
def cmd_status(args):
    """项目/模块状态。"""
    sys.path.insert(0, str(WORKSTUDY))
    from aitest.agent_scheduler import recommend_next_agent, check_preconditions
...
def cmd_agent(args):
    """Agent 调度 + 执行。"""
    sys.path.insert(0, str(WORKSTUDY))
    from aitest.agent_scheduler import check_preconditions, recommend_next_agent, auto_advance

    module = args.module or "tank"

    if args.action == "run":
        # ── agent run: 执行 Agent（使用 agent_runner）──
        from aitest.agent_runner import run_agent, list_agents
...
def cmd_rag(args):
    ...
    if args.action == "search":
        from aitest.rag_engine import search_known_issues, search_context
        ...
        from aitest.rag_engine import index_all
...
def cmd_bus(args):
    """事件总线。"""
    sys.path.insert(0, str(WORKSTUDY))
    from aitest.event_bus import emit, list_pending, list_all, process_pending
...
def cmd_dashboard(args):
    ...
    from aitest.agent_scheduler import AGENT_TRIGGERS
    from aitest.rag_engine import get_chroma_client
    ...
    if (CONTEXT_MODULES).exists():
        ...
        for mod_dir in sorted(CONTEXT_MODULES.iterdir()):
            ...
            # 判断当前 Phase
            from aitest.agent_scheduler import recommend_next_agent
            try:
    ...
    from aitest.event_bus import list_pending, EVENT_DIR
    ...
    try:
        from aitest.bug_history import list_bugs as list_bugs_fn, BUG_DB
...
def cmd_errors(args):
    """错误日志查看与清理 (P0-2)。"""
    sys.path.insert(0, str(WORKSTUDY))
    from aitest.error_logger import list_recent, get_summary, cleanup_old
...
def cmd_bug(args):
    """Bug 历史库。"""
    sys.path.insert(0, str(WORKSTUDY))
    from aitest.bug_history import (
...
def cmd_testcase(args):
    """测试用例 Excel 导出。"""
    sys.path.insert(0, str(WORKSTUDY))
    from aitest.testcase_exporter import export_testcases_to_excel
...
def cmd_kpi(args):
    ...
    sys.path.insert(0, str(WORKSTUDY))
    from aitest.governance_kpi import run_kpi_summary
    ...
    if args.action == "summary":
        ...
        from aitest.governance_kpi import export_to_excel
...
def cmd_regression(args):
    """回归测试运行器 (P1-4)"""
    sys.path.insert(0, str(WORKSTUDY))
    from aitest.regression import RegressionRunner
...
def cmd_ab(args):
    ...
    if args.action == "list":
        ...
        from aitest.ab_test import ABTestRunner
...
def cmd_eval(args):
    """Skill 评估运行器 (P1-2)"""
    sys.path.insert(0, str(WORKSTUDY))
    from aitest.evaluator import EvalRunner
...
def cmd_trace(args):
    ...
    sys.path.insert(0, str(WORKSTUDY))
    from aitest.trace import query_trace_events, get_trace_summary, cleanup_old_traces, get_run_stats, get_cost_leaderboard
