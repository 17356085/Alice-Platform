...
用法:
    python -m aitest.testcase_exporter sales customer
...
def parse_test_cases_md(md_path: Path, module_name: str = "客户管理模块") -> list[TestCaseRow]:
    ...
    # 解析优先级段 (## P0 / ## P1 / ## P2)
    priority_sections = re.split(r'\n## P[012]', text)

    for section in text.split('\n## '):
        if not section.startswith('P'):
            continue
        priority = section[0:2]  # P0/P1/P2
        content = section[2:]
        ...
        for line in content.split('\n'):
            ...
            if not line.startswith('| TC-'):
                continue
            # 解析: | TC-CUST-001 | 标题 | P0 | 前置条件 | 步骤 | 数据 | 预期 | 状态 |
            ...
            priority_val = cols[2] if len(cols) > 2 else priority
            precondition = cols[3] if len(cols) > 3 else ""
            steps = cols[4] if len(cols) > 4 else ""
            test_data = cols[5] if len(cols) > 5 else ""
            expected = cols[6] if len(cols) > 6 else ""
            auto_status = cols[7] if len(cols) > 7 else ""
