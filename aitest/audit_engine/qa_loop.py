...
使用:
    from aitest.audit_engine.qa_loop import QALoop
...
class QALoop:
    ...
    def __init__(
        ...
        self._script_dir = zjsn / "script" / module
    ...
    def _collect_pytest_output(self, failed_test: str = None) -> str:
        ...
        if not zjsn:
            return ""
        log_files = sorted(zjsn.glob("*.log"), key=os.path.getmtime, reverse=True)
        if log_files:
    ...
    def _rerun(self, failed_test: str = None, extra_args: str = None) -> tuple[bool, str]:
        ...
        zjsn_cwd = get_test_project_root()
        if not zjsn_cwd:
            return QAResult(module=self.module, status=LoopStatus.FAILED, rounds=0,
                           error="No test project configured. Use aitest project set --id=<project>.")
        try:
            proc = subprocess.run(
                cmd,
                cwd=str(zjsn_cwd),
