...
用法:
    from aitest.audit_engine.online_monitor import OnlineMonitor, collect_run_metrics
...
def _resolve_kpi_data_dir() -> Path:
    """Resolve KPI data dir: .tlo/runtime/kpi/timeseries/ → legacy governance/kpi/timeseries/."""
    try:
        from aitest.platform.paths import get_tlo_dir
        tlo = get_tlo_dir()
        if tlo:
            return tlo / "runtime" / "kpi" / "timeseries"
    except Exception:
        pass
    return GOVERNANCE / "kpi" / "timeseries"

KPI_DATA_DIR = _resolve_kpi_data_dir()
