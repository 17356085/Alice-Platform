...
class TestApprovalTodo:
    @pytest.fixture(autouse=True)
    def _reset_after_each(self, driver_setup):
        yield
        try:
            ApprovalTodoPage(driver_setup).click_reset()
        except Exception:
            pass
    ...
    def test_sy_todo_03_search_by_status(self, driver_setup):
        page = ApprovalTodoPage(driver_setup)
        case("SY-TODO-03", "按审批状态筛选")
        step("点击重置")
        page.click_reset()
        step("选择状态：待审批")
        try:
            page.select_status("待审批")
    ...
    def test_sy_todo_04_search_by_date(self, driver_setup):
        page = ApprovalTodoPage(driver_setup)
        case("SY-TODO-04", "按日期范围搜索")
        step("点击重置")
        page.click_reset()
        step("设置日期范围")
    ...
    def test_sy_todo_05_reset_button(self, driver_setup):
        page = ApprovalTodoPage(driver_setup)
        case("SY-TODO-05", "重置按钮功能正常")
        step("输入筛选条件")
        page.input_title("test")
    ...
    def test_sy_todo_06_pagination(self, driver_setup):
        page = ApprovalTodoPage(driver_setup)
        case("SY-TODO-06", "分页跳转")
        step("点击重置")
        page.click_reset()
        step("记录第一页页码")
    ...
    def test_sy_todo_07_detail_view(self, driver_setup):
        page = ApprovalTodoPage(driver_setup)
        case("SY-TODO-07", "查看审批详情")
        step("校验列表是否有数据")
        if page.get_table_row_count() == 0:
            pytest.skip(page.get_empty_text() or "当前无待审批数据，跳过详情验证")
        step("点击第一行详情")
        try:
    ...
    def test_sy_todo_08_approve_action(self, driver_setup):
        page = ApprovalTodoPage(driver_setup)
        case("SY-TODO-08", "审批通过操作")
        step("校验列表是否有数据")
        if page.get_table_row_count() == 0:
            pytest.skip(page.get_empty_text() or "当前无待审批数据，跳过审批操作验证")
        step("点击第一行通过按钮")
        try:
            page.click_row_action(1, "通过")
        except TimeoutException:
            pytest.skip("当前列表第一行无可用的通过按钮")
        step("填写审批意见并确认")
        try:
            page.fill_approval_comment("[AUTO] 自动化测试-审批通过")
            page.click_approval_confirm()
        except TimeoutException:
            pass
        msg = page.wait_for_toast_text(timeout=6)
        ok = (not msg) or any(k in (msg or "") for k in ["成功", "通过", "完成"])
        assert ok, ea("审批操作已触发", msg or "未获取到提示")
