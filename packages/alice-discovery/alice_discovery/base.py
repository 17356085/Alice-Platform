...
class BaseDiscovery(ABC):
    ...
    def __init__(self, project_id: str, output_root: Path = None):
        """
        Args:
            project_id: 项目标识
            output_root: 输出根目录（默认: .discovery/ 在当前目录）
        """
    ...
    async def run_full_discovery(self) -> DiscoveryIndex:
        ...
        if self._output_root:
            discovery_dir = self._output_root / ".discovery"
        else:
            discovery_dir = Path.cwd() / ".discovery"
