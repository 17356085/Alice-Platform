...
def _get_project_dir() -> Path:
    ...
    return Path(os.environ.get("AITEST_PROJECT_DIR", str(_WORKSTUDY)))
...
def resolve_artifact_path(
    ...
    if agent_name in dev_agent_map:
        module_dir = str(_GOVERNANCE / "context" / "projects" / "dev-platform")
    else:
        module_dir = str(_CONTEXT_MODULES / module)
...
def resolve_path(
    ...
    if not Path(resolved).is_absolute() and not resolved.startswith(str(_WORKSTUDY)):
        resolved = str(_WORKSTUDY / resolved)
