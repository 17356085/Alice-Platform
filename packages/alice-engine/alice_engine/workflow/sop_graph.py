...
WORKSTUDY = _Path(".")
CONTEXT_MODULES = WORKSTUDY / "context"
...
def route_next_phase(state: SOPState) -> str:
    ...
    特殊规则:
      ...
      - Bug Analysis 后进入 qa_loop_decision 处理 QA Loop 状态机
    ...
    H5 fix: 路由函数现在是纯函数 — 不修改 state，不写文件。
    QA Loop 状态管理移到了 qa_loop_decision_node。
    """
    # 致命错误 → 直接退出
...
# ══════════════════════════════════════════════════════════════════════════
#  H5: QA Loop Decision Node — 独立节点处理 Bug Analysis 后的状态机
# ══════════════════════════════════════════════════════════════════════════
