...
def analyze_fail_node(state: SOPState) -> dict:
    ...
    # ── RAG 匹配已知问题 (本模块 + 跨模块) ──
    # 使用 Engine 注入的 knowledge_context，不依赖外部平台
    rag_matches = []
    cross_module_hits = []
    page_hits = []
    ...
    if knowledge_context:
        # 从 knowledge_context 中提取相关知识
        for page_key, items in knowledge_context.items():
            for item in items:
                content = getattr(item, "content", str(item))
                metadata = getattr(item, "metadata", {})
                entry = {"document": content, "metadata": metadata}
                if page_key == page:
                    rag_matches.append(entry)
                elif metadata.get("module", "") != module:
                    cross_module_hits.append(entry)
