...
def configure_behavior_pack(pack):
    """配置行为包（由 Engine 初始化时调用）。"""
    global _BEHAVIOR_PACK
    _BEHAVIOR_PACK = pack


def get_behavior_pack():
    """获取当前行为包。"""
    return _BEHAVIOR_PACK
...
# 产物路径 — 通过 behavior pack 解析，不硬编码 governance 路径
def _get_kpi_reports() -> Path:
    pack = get_behavior_pack()
    if pack and pack.kpi_dir:
        return pack.kpi_dir / "reports"
    return _PATH_BASE / "kpi" / "reports"

def _get_kpi_testcases() -> Path:
    pack = get_behavior_pack()
    if pack and pack.kpi_dir:
        return pack.kpi_dir / "testcases"
    return _PATH_BASE / "kpi" / "testcases"

def _get_execution_reports() -> Path:
    pack = get_behavior_pack()
    if pack and pack.artifacts_dir:
        return pack.artifacts_dir / "execution-reports"
    return _PATH_BASE / "artifacts" / "execution-reports"
...
def _check_report_artifacts(module: str, pages: list[str]) -> list[tuple[bool, str, str]]:
    ...
    exec_reports = _get_execution_reports()
    pattern = f"TEST_EXECUTION_{module.upper()}_*.md"
    found = list(exec_reports.glob(pattern)) if exec_reports.exists() else []
    ok = len(found) > 0
    results = [(ok, str(exec_reports / pattern), f"执行报告 ({pattern})")]
    return results
...
def _check_knowledge_artifacts(module: str, pages: list[str]) -> list[tuple[bool, str, str]]:
    ...
    kpi_reports = _get_kpi_reports()
    kpi_testcases = _get_kpi_testcases()
    results = []
    for page in pages:
        xlsx_path = kpi_reports / module / f"测试报告-{module}-{page}.xlsx"
        md_path = kpi_testcases / module / f"testcases-{module}-{page}.md"
