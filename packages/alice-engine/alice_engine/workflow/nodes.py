...
def make_agent_loop_node(
    ...
    def agent_loop_node(state: dict) -> dict:
        ...
        try:
            agent = AgentLoop(
                agent_name,
                provider=state.get("provider") or config.resolve_llm_provider(),
                module=state["module"],
                page=page,
                verbose=False,
            ...
            logger.info(f"  [{agent_name}] Done: success={loop_state.success}, "
                        f"skills={len(loop_state.completed_skills)}/{len(agent.skills)}, "
                        f"steps={loop_state.step}")
