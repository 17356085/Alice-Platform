...
# 路径可通过 configure() 覆盖，或由调用方传入
_WORKSTUDY: Path | None = None
_GOVERNANCE: Path | None = None


def configure(workstudy: Path = None, governance: Path = None):
    """配置路径（可选，不调则使用默认）。"""
    global _WORKSTUDY, _GOVERNANCE
    if workstudy:
        _WORKSTUDY = Path(workstudy)
    if governance:
        _GOVERNANCE = Path(governance)


def _get_workstudy() -> Path:
    return _WORKSTUDY or Path(".")


def _get_governance() -> Path:
    return _GOVERNANCE or _get_workstudy() / "governance"
