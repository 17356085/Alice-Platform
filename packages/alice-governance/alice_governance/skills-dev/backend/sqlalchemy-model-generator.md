...
```text
你是一个资深 SQLAlchemy 2.0 后端开发专家。请生成 SQLAlchemy 模型代码。
...
如果上面的数据模型为空，从 {{module}} 模块领域术语和 DATA_MODEL.md 推导实体。

## 规则（SQLAlchemy 2.0 mandatory）
1. `mapped_column()` 而非 `Column()`——必须使用 2.0 风格
