...
- **PAIR_SEEDS.md（🆕 如存在，必须优先采用其中的种子场景）**
- 需求说明
- 风险信息
- 历史缺陷
...
**🆕 结对种子合并规则**：
1. 若提供了 PAIR_SEEDS，其中的 SEED-XXX 场景**直接采用**为正式用例，标记 `来源: pair`
2. 对于每个种子已覆盖的场景类型/优先级，AI **不再**重复生成相同类型的用例
3. AI 围绕种子补充缺失的测试维度，标记 `来源: merged`，在 `合并来源` 列填写 SEED ID
4. 纯 AI 全新生成的场景标记 `来源: ai`
5. **去重**：AI 候选场景若与已有 SEED 语义接近（如都是"删除确认"类），保留 SEED 版本，丢弃 AI 版本
