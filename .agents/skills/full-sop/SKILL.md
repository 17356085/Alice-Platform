...
**不再是 Codex 调 Codex**。编排引擎是 Python LangGraph，LLM 调用走 `run_skill()` → Anthropic API。
...
| Project Init | AgentLoop → Codex API | PROJECT_CONTEXT.md |
| Requirement | AgentLoop → Codex API | MODULE_CONTEXT.md |
| Test Design | AgentLoop → Codex API (per page) | PAGE_CONTEXT, RISK_MODEL, TEST_CASES |
| Automation | AgentLoop → Codex API (per page) | TECH_ANALYSIS, PageObject, test_*.py |
...
| Report | AgentLoop → Codex API | TEST_SUMMARY.md |
| Knowledge | AgentLoop → Codex API | known-issues.yaml 更新 |
...
- ⚠️ HITL: `--non-interactive` 模式自动批准（Codex 路径无交互终端）
